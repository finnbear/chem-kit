# Physics Settings
min_atomic_weight = 1
max_initial_atomic_weight = 25
max_atomic_weight = 200
min_energy = 0
max_energy = 255
...
min_temperature = 0
max_temperature = 70
temperature_damper = 0.80
fusion_temperature_change = 800
fusion_area, fusion_ammount, fusion_radius = 0, 0, 15
fission_temperature_change = 8000
fission_area, fission_amount, fission_radius = 0, 0, 25
...
atomic_weight_display_offset = 30 # Must be less than or equal to 255 - max_atomic_weight
        ...
        for y in range(0, canvas_size):
            ...
            if atom_weight >= min_atomic_weight:
                ...
                if original_weight >= min_atomic_weight:
                    ...
                    if original_weight == atom_weight and (original_weight + atom_weight) < max_atomic_weight and (original_energy > fusion_threshold or atom_energy > fusion_threshold):
                        # Fusion
                        for x2 in range(x - fusion_radius, x + fusion_radius):
                            for y2 in range(y - fusion_radius, y + fusion_radius):
                                try:
                                    new_canvas[x2, y2]['temperature'] += int(fusion_ammount * random.random())
                                    new_canvas[x2, y2]['temperature'] = min(max_temperature, new_canvas[x2, y2]['temperature'])
                                except KeyError:
                                    z = 0
                        new_canvas[new_x, new_y] = {'weight': atom_weight * original_weight, 'energy': atom_energy + original_energy, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                    elif original_weight > 2 * atom_weight:
                        # Fission
                        for x2 in range(x - fission_radius, x + fission_radius):
                            for y2 in range(y - fission_radius, y + fission_radius):
                                try:
                                    new_canvas[x2, y2]['temperature'] += int(fission_ammount * random.random())
                                    new_canvas[x2, y2]['temperature'] = min(max_temperature, new_canvas[x2, y2]['temperature'])
                                except KeyError:
                                    z = 0
                        new_canvas[new_x, new_y] = {'weight': original_weight / 2, 'energy': original_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                        new_canvas[new_x + random.choice([-1.0, 1.0]), new_y] = {'weight': original_weight / 2, 'energy': original_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                        new_canvas[new_x, new_y + random.choice([-1.0, 1.0])] = {'weight': atom_weight, 'energy': atom_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                    elif atom_weight > 2 * original_weight:
                        # Fission
                        for x2 in range(x - fission_radius, x + fission_radius):
                            for y2 in range(y - fission_radius, y + fission_radius):
                                try:
                                    ...
                                    z = 0
                        new_canvas[new_x, new_y] = {'weight': atom_weight / 2, 'energy': atom_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                        new_canvas[new_x + random.choice([-1.0, 1.0]), new_y] = {'weight': atom_weight / 2, 'energy': atom_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                        new_canvas[new_x, new_y + random.choice([-1.0, 1.0])] = {'weight': original_weight, 'energy': original_energy / 2, 'temperature': newTemperature(canvas[x, y]['temperature'])}
                else:
                    # Move atom to resulting position
                    new_canvas[new_x, new_y] = {'weight': atom_weight, 'energy': atom_energy, 'temperature': newTemperature(canvas[x, y]['temperature'])}
